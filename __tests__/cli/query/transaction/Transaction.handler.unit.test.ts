...
describe("QueryTransactionHandler", () => {
    ...
    const functionSpy = jest.spyOn(Get, "queryTransaction");

    beforeEach(() => {
        functionSpy.mockClear();
        // defaultReturn.response.records[transactionName.toLowerCase()] = [{prop:"test1"}, {prop:"test2"}];
        functionSpy.mockImplementation(async () => defaultReturn);
    });

    it("should call the query transaction api", async () => {
