...
export class TempTestProfiles {
    ...
    public static async createProfiles(testEnvironment: ITestEnvironment, profileTypes: string[] = []) {
        const profileNames: { [key: string]: string[] } = {ims: []};
        this.log(testEnvironment, "Creating the following profileTypes: " + profileTypes);
        for (const type of profileTypes) {
            if (type === "ims") {
                profileNames.ims.push(await TempTestProfiles.createImsProfile(testEnvironment));
            } else {
     ...
     */
    private static async createImsProfile(testEnvironment: ITestEnvironment) {
        ...
        this.log(testEnvironment, `Created ims profile '${profileName}'. Stdout from creation:\n${output.stdout.toString()}`);
        return profileName;
    }
