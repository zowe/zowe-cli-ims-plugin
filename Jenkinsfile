 ...
 */
def GIT_REPO_URL = 'https://github.com/zowe/zowe-cli-ims-plugin.git'
                ...
                timeout(time: 5, unit: 'MINUTES') {
                    ...
                    withCredentials([usernamePassword(credentialsId: ARTIFACTORY_CREDENTIALS_ID, usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                        script {
                            ...
                            // Install the plugin
                            if (BRANCH_NAME == MASTER_BRANCH) {
                                sh "bright plugins install ${TARGET_SCOPE}/ims@next"
                            } else {
                                sh "bright plugins install ${TARGET_SCOPE}/ims@latest"
                            }
