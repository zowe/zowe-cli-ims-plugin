...
import { IIMSApiResponse } from "../doc/IIMSApiResponse";
import { ImsMessages } from "../constants/Ims.messages";
...
export class ImsRestClient extends RestClient {
    ...
    public static async getExpectParsedXml(session: AbstractSession,
                                           resource: string, reqHeaders: any[] = []): Promise<IIMSApiResponse> {
        const data = await ImsRestClient.getExpectString(session, resource, reqHeaders);
        const apiResponse = ImsRestClient.parseStringSync(data);
        return ImsRestClient.verifyResponseCodes(apiResponse);
    }
    ...
    public static async deleteExpectParsedXml(session: AbstractSession,
                                              resource: string, reqHeaders: any[] = []): Promise<IIMSApiResponse> {
        const data = await ImsRestClient.deleteExpectString(session, resource, reqHeaders);
        const apiResponse = ImsRestClient.parseStringSync(data);
        return ImsRestClient.verifyResponseCodes(apiResponse);
    }
    ...
    public static async putExpectParsedXml(session: AbstractSession,
                                           ...
                                           payload: any): Promise<IIMSApiResponse> {
        if (payload != null) {
            payload = ImsRestClient.convertPayloadToXML(payload);
        }
        const data = await ImsRestClient.putExpectString(session, resource, reqHeaders, payload);
        const apiResponse = ImsRestClient.parseStringSync(data);
        return ImsRestClient.verifyResponseCodes(apiResponse);
    }
    ...
    public static async postExpectParsedXml(session: AbstractSession,
        ...
        const data = await ImsRestClient.postExpectString(session, resource, reqHeaders, payload);
        const apiResponse: IIMSApiResponse = ImsRestClient.parseStringSync(data);
        return ImsRestClient.verifyResponseCodes(apiResponse);
    }
     ...
     * @param {IIMSApiResponse} apiResponse - the parsed response
     * @returns {IIMSApiResponse} - the response if it was correct
     * @throws {ImperativeError} request did not get the expected codes
     */
    private static verifyResponseCodes(apiResponse: IIMSApiResponse): IIMSApiResponse {
        if (apiResponse.response != null && apiResponse.response.resultsummary != null
            && apiResponse.response.resultsummary.api_response1 === ImsRestClient.IMS_SUCCESS_RESPONSE_1
            && apiResponse.response.resultsummary.api_response2 === ImsRestClient.IMS_SUCCESS_RESPONSE_2) {
            // expected return code and reason code specify
            return apiResponse;
        } else {
            throw new ImperativeError({
                msg: ImsMessages.cmciRequestFailed.message + "\n" + TextUtils.prettyJson(apiResponse),
            });
        }
    }
    ...
    protected processError(original: IImperativeError): IImperativeError {
        ...
        try {
            const xmlDetails = ImsRestClient.parseStringSync(details);
